...
from interpCoords_4km_conservative import interpCoords_4km_nearest_conservative
...
for date in date_list: 
    for version in versions:
        ...
        for member in members:
            ...
            filename = f"ENS-{version}-epoch{epoch}-{date}-subset-{member}.nc"
            ...
            tp_interp = interpCoords_4km_nearest_conservative(x_forecast, lat_ai, lon_ai, lat_grid, lon_grid, var='tp')
            tp_interp = tp_interp.clip(min=0, keep_attrs=True)  # Clip negative values to zero
...
end_time = datetime.now() # time.time()
